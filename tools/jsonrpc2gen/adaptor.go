...
func (a*%[3]sJSONRPCAdaptor) Call(
        ...
        defer func (){
                jsonrpcErr = jsonrpc2.AnalysisError(err, errorData,methodErr, paramsUnmarshal,call,errorDataUnmarshal)
                return
        }()
...
{{range .}}
        ...
        if method == {{getJsonName .Comment}} {
                ...
                {{end}}
                tmpResult, {{if hasErrorData .}}tmpErrorData, {{end}}methodErr = a.service.{{$name}}(ctx{{if hasParams .}}, tmpParams{{end}})
                call = true
                {{if hasErrorData .}}
                if tmpErrorData != nil {
...
var adaptorTemplateHasErrorData = func(f *ast.Field) bool {
	return len(f.Type.(*ast.FuncType).Results.List) == 3
}
